...
const startToken = '<div className="services-grid">';
const endToken = '</div>\n        </div>\n    </section>';
...
if (startIndex !== -1 && endIndex !== -1) {
    ...
    const magicBento = `<div className="services-grid" style={{ display: 'block', padding: '0', maxWidth: '100%' }}>
                <MagicBento 
                    cards={serviceCards}
                    textAutoHide={false}
                    enableStars={true}
    ...
    content = before + magicBento + after;
    fs.writeFileSync('src/pages/Index.jsx', content);
    console.log('Successfully replaced services-grid');
} else {
